...
def Model(input_layers, output_layer, 
    ...
    """
    It implements a two GRU network (uni or bi directional) to predict the
    relation between two arguments.

    Architecture:
    argA -> embedding -> GRU -> dropout |
                                    insert hidden state to GRU
             argB -> embedding ->             GRU -> dropout -> dense -> dense ->softmax
    ...
    if lstm == "bi":
        x = Bidirectional(RU(n_lstm, 
                           activation=acti_lstm))(x) 
    else:
        x = GRU(n_lstm, 
                 activation=acti_lstm)(x) 
    ...
    if lstm == "bi":
        y = Bidirectional(GRU(n_lstm, 
                           activation=acti_lstm))(y, initial_state=state) 
    else:
        y = GRU(n_lstm, 
                 activation=acti_lstm)(y, initial_state=state) 
    ...
    y = Dense(n_nn, activation="relu")(y) 

    #z = Dense(n_classif, activation="softmax")(z)
    y = Dense(n_classif, activation="sigmoid")(y)

    y = output_layer(y)
    model = keras.models.Model(inputs=[*x.input, *y.input], outputs=y)
